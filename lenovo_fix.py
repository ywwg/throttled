...
def power_thread(config, regs, exit_event):
    ...
    while not exit_event.is_set():
        ...
        if mchbar_mmio is not None:
            write_value = int(config.getfloat(
                power['source'], 'PL1_Tdp_W', fallback=None)) * 1000000
            with open('/sys/devices/virtual/powercap/intel-rapl-mmio/intel-rapl-mmio:0/constraint_0_power_limit_uw', 'w') as f:
                f.write('%d' % write_value)
            if args.debug:
                with open('/sys/devices/virtual/powercap/intel-rapl-mmio/intel-rapl-mmio:0/constraint_0_power_limit_uw', 'r') as f:
                    read_value = int(f.read())
                    match = OK if write_value == read_value else ERR
                    log(
                        '[D] MCHBAR PACKAGE_POWER_LIMIT - write {:#d} - read {:#d} - match {}'.format(
                            write_value, read_value, match
                        )
                    )
...
def main():
    ...
    parser.add_argument('--nofix', action='store_true',
                        help="Don't actually perform fixes")
    parser.add_argument('--config', default='/etc/lenovo_fix.conf',
                        help='override default config file path')
    parser.add_argument('--force', action='store_true',
                        help='bypass compatibility checks (EXPERTS only)')
    parser.add_argument('--log', metavar='/path/to/file',
    ...
    if args.monitor is not None:
        monitor_thread = Thread(
            target=monitor, args=(exit_event, args.monitor))
